...
class Poolopt_on_Corrmat(nn.Module):
    ...
    def forward(self, corr):
        ...
        for bs_idx in range(bs):
            sort_corr_tmp = torch.index_select(
                torch.unsqueeze(corr[bs_idx], 0), 1, sort_v_idx[bs_idx]
            )
            if bs_idx == 0:
...
class Inception_pool(nn.Module):
    def __init__(self, in_channels=8, nb_inceptors=32):
        ...
        self.branch_pool = BasicConv2d(
            in_channels, nb_inceptors, kernel_size=1
        )
        ...
        self.branch3x3dbl_2 = BasicConv2d(
            nb_inceptors, nb_inceptors, kernel_size=3, padding=1
        )
        ...
        self.branch5x5_2 = BasicConv2d(
            nb_inceptors, nb_inceptors, kernel_size=5, padding=2
        )

        self.final_conv = BasicConv2d(
            nb_inceptors * 4, nb_inceptors, kernel_size=3, padding=1
        )
...
class Inception_mask_small(nn.Module):
    def __init__(self, in_channels=8, nb_inceptors=4):
        ...
        self.branch3x3 = BasicConv2d(
            in_channels, nb_inceptors, kernel_size=3, padding=1
        )

        self.branch5x5 = BasicConv2d(
            in_channels, nb_inceptors, kernel_size=5, padding=2
        )
...
class Inception_mask_large(nn.Module):
    def __init__(self, in_channels=8, nb_inceptors=4):
        ...
        self.branch7x7 = BasicConv2d(
            in_channels, nb_inceptors, kernel_size=7, padding=3
        )

        self.branch11x11 = BasicConv2d(
            in_channels, nb_inceptors, kernel_size=11, padding=5
        )
...
def base_feature():
    ...
    model = nn.Sequential()
    [
        model.add_module(name=str(i), module=layer)
        for i, layer in enumerate(list(vgg.features)[:-7])
    ]
    return model
...
class dmvn(nn.Module):
    ...
    def forward(self, x2, x1):
        ...
        # output masks
        out1 = self.classifier(corr1)
        out2 = self.classifier(corr2)

        out1 = F.interpolate(
            out1, size=(h, w), mode="bilinear", align_corners=True
        )
