...
class Convblock(nn.Module):
    def __init__(
        self, in_channels, out_channels, padding_, dilation_, batch_norm=False
    ):
...
class DMAC_VGG_Module(nn.Module):
    ...
    def __init__(self, block, NoLabels, gpu_idx, h, w):
        ...
        """
        self.classifier = self._make_pred_layer(
    ...
    def forward(self, x1, x2):
        ...
        c1_3, c2_3 = corr_fun(
            x1_3, x2_3, self.Corr, self.poolopt_on_corrmat, self.new_indices
        )

        x1_4 = self.Scale_4(x1_3)
        x2_4 = self.Scale_4(x2_3)
        c1_4, c2_4 = corr_fun(
            x1_4, x2_4, self.Corr, self.poolopt_on_corrmat, self.new_indices
        )

        x1_5 = self.Scale_5(x1_4)
        x2_5 = self.Scale_5(x2_4)
        c1_5, c2_5 = corr_fun(
            x1_5, x2_5, self.Corr, self.poolopt_on_corrmat, self.new_indices
        )

        c1 = torch.cat((c1_3, c1_4, c1_5), 1)
        c2 = torch.cat((c2_3, c2_4, c2_5), 1)

        out1 = self.classifier(c1)
        out2 = self.classifier(c2)
