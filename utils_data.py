...
def load_pairs_csv(input_pairs_csv_file, root):
    ...
    if not os.path.isfile(input_pairs_csv_file):
        raise IOError(
            "ERROR: cannot locate input splicing task csv file %s"
            % input_pairs_csv_file
        )
    ...
    for line in lines:
        ...
        if not flag:
            continue
        pair_list.append(
            (lut["image1"], lut["image2"], lut["label"], lut["gt1"], lut["gt2"])
        )
    return pair_list
...
def load_pairs(data_path, subpath_list, root):
    pair_list = []
    for subpath in subpath_list:
        list_path = os.path.join(data_path, "labelfiles", subpath)
        pair_list_ = load_pairs_csv(list_path, root)
