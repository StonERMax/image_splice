...
if __name__ == "__main__":
    ...
    # model
    model = models_conv.CustomModelCMFDSim()
    ...
    data_test2 = torch.utils.data.DataLoader(
        dataset_test2, batch_size=args.batch_size, shuffle=True, num_workers=4
    )
    ...
    if not os.path.exists("./ckpt_scratch_cmfd"):
        os.mkdir("./ckpt_scratch_cmfd")
    ...
    for ep in tqdm(range(init_ep, args.max_epoch)):
        ...
        for ret in data_train:
            ...
            if iteration % 100 == 0:
                ...
                test.test_cmfd(
                    data_test, model, args, iteration=None, device=device, logger=None, num=5
                )
                ...
                torch.save(
                    ...
                    os.path.join("./ckpt_scratch_cmfd", model_name + ".pkl"),
                )
                print(Fore.RED + f"weight saved in {model_name}.pkl")

            
            if iteration % 100 == 0:
                print(Fore.YELLOW + "Test on CASIA:")
                test.test_cmfd(
                    data_test2, model, args, iteration=None, device=device, logger=None, num=5
                )
                print(Fore.YELLOW + "----" * 5)
